...
// Contraste : Marine #1C2B3A sur #FAFAF8 = ratio 12.6:1 (WCAG AAA)
</script>

<template>
  <div class="hero-riviera min-h-screen bg-[#FAFAF8] relative overflow-hidden flex flex-col justify-center px-6 md:px-20 py-20">
    <div class="grid md:grid-cols-2 gap-12 items-center max-w-7xl mx-auto w-full">
      ...
      <div class="order-2 md:order-1 space-y-10 text-center md:text-left animate-slideRight">
        <div class="inline-block bg-[#2E6E8E] text-white px-4 py-1 text-[10px] font-bold tracking-[0.4em] uppercase">
          SAVE THE DATE
        </div>

        <h1 class="template-title font-serif italic text-[#1C2B3A]">
          {{ displayNames }}
        </h1>

        <p class="template-body text-[#4A6572] max-w-md font-light leading-relaxed">
          {{ content.intro_text || 'Nous serions honorés de votre présence pour célébrer notre union.' }}
        </p>

        <div class="space-y-2 pt-6">
          <p class="font-mono text-xs text-[#2E6E8E] tracking-tighter">{{ content.date_display || '15.06.2026' }}</p>
          <p v-if="content.address" class="template-label text-[#1C2B3A]">{{ content.address }}</p>
        </div>
      </div>
      ...
      <div class="order-1 md:order-2 flex justify-center relative overflow-hidden">
        <div class="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none scale-150">
           <span class="text-[25vw] font-serif italic text-[#1C2B3A]">{{ content.monogram || 'E' }}</span>
        </div>
        
        <svg class="w-full max-w-[320px] h-auto" viewBox="0 0 200 400" fill="none">
          <!-- Branche principale -->
          <path d="M100 400 Q120 300 80 200 T100 0" stroke="#2E6E8E" stroke-width="1" class="olive-path" :class="{ 'drawn': isDrawn }" />
          <!-- Feuilles -->
...
.hero-riviera { font-family: 'Jost', sans-serif; }
.template-title { font-family: 'Libre Baskerville', serif; font-size: clamp(3rem, 10vw, 5.5rem); line-height: 1.1; }
